...
**Область индексации:** 7 разделов основного ТЗ + 7 приложений + ADR-карточки (`приложения к ТЗ/ADR/`, 3 файла) + `ARCHITECTURE.md` (производный).
...
| `приложения к ТЗ/ADR/` | Адресные карточки принятых решений `ADR-001`, `ADR-002`, `ADR-003` — дословный перенос из реестра; сами нормы — в ТЗ (`02` §3a, `03` §9b, `03` §8a) |
...
| `корзина/` | Архив |
| `REVIEW-REGISTRY.md` | Реестр решений по замечаниям (отдельный слой) |
