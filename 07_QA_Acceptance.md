...
- **Идентификатор baseline в задаче.** Context Header задачи обязан нести идентификатор baseline (`BASELINE_ID` или эквивалент), когда baseline введён. До его введения версии применимых документов фиксируются в полях `INPUTS`/`CONTRACT`.
