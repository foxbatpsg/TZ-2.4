...
**Baseline:** `TZ-2.6 @ cd08d26` (ревизия v2.6 — перенос решения `Q-06` в нормативный корпус: вариант A, норма в `05` §4b, контракт в `MCP TOOL CONTRACTS` §3; 2026-09-16), ветка `master`.
...
| **Q-07** | Provenance доказательств из локальных файлов | — утверждённого определения в корпусе нет; проект: `OPEN-QUESTIONS-Q-06-Q-07.md`, раздел Q-07 (ненормативный) | карточки Astra1: Глава 2 стр.100, 106; Глава 5 стр.123, 137; Глава 7 стр.302, 338 (зависимости) | ⛔ **OPEN**; `status: open` — требуется решение владельца; зависимые задачи `BLOCKED`; `REVIEW-REGISTRY.md` §8 |
...
> **О нумерации.** `Q-01`…`Q-13` непрерывны; пропусков нумерации нет. Решение `Q-06` перенесено в нормативный корпус 2026-09-16 (строка выше); решение `Q-07` до корпуса не донесено — вопрос остаётся OPEN. `Q-09`…`Q-12` отсутствовали в нормативном корпусе и использовались только как блокеры в карточках Astra1 (класс `D-5`) — приняты 2026-09-15 из черновика `корзина/Утверждённые изменения… v1.1` с доработкой (строки выше). `Q-13` введён решением архитектора (2026-09-13) — см. ниже.
...
| MCP Tool Contracts | `MCP TOOL CONTRACTS v1.1.md` | 23 инструмента, полные контракты; §3 `search_within_document` — опция `include_evidence_context`, 32 KiB (Q-06, 2026-09-16) |
...
| **D-2** | `Q-06`/`Q-07` — открытые решения, не перенесённые в нормативный корпус | проекты: `OPEN-QUESTIONS-Q-06-Q-07.md`; регистрация: `REVIEW-REGISTRY.md` §8 | ⛔ **OPEN** — частично: `Q-06` закрыт 2026-09-16 (вариант A перенесён: `05` §4b, `MCP TOOL CONTRACTS` §3); `Q-07` остаётся открытым — требуется утверждение владельца; зависимые карточки `BLOCKED` до его решения; ложная классификация снята 2026-09-16 |
