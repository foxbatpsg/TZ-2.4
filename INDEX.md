...
| `07_QA_Acceptance.md` | Раздел 7. QA и критерии приёмки | 25 КБ | Общие требования (§1), Data isolation (§2), Query Planner (§3), URL normalization (§4), Deduplication (§5), Ranking (§6), BM25 (§7) + RU-морфология (§7.1), Chunking (§8), Evidence (§9), Observation/Aggregation (§10), Claims (§11), Contradiction (§12), Research Gap (§13), **Source independence (§14)**, Adaptive Search (§15), Local-First (§16), Cache (§17), **DB-write layer (§18)**, **MCP (§19) + защита stdout (§19.1)**, LLM resilience (§20), JSON degradation (§21), GUI (§22) + кооперативная отмена (§22.1), Network resilience (§23), Security (§24), **Acceptance Criteria (§25)**, **MVP/Post-MVP (§26)**, Definition of Done (§27), фикстуры и golden datasets (§28) |
...
> **A-01 восстановлен** (2026-09-13): каноническая формулировка найдена в старых файлах проекта. См. `REVIEW-REGISTRY.md` §D-4. Статус — `active`, ожидает внесения в нормативный состав.

| ID | Название | Первичный источник | Упоминания | Связь с реестром |
|---|---|---|---|---|
| **A-01** | Редакционные ошибки Roadmap и граф зависимостей | ⟂ **восстановлено из старых файлов** (см. D-4) | EPIC SPEC стр.146; ROADMAP стр.6, 244, 575; EPIC→TASK стр.5, 269; TASK EXEC стр.14 | **D-4 (RESOLVED)** |
...
| **A-17** | Приватность локальных источников: пути не передаются LLM и в отчёт | `02` стр. 100 | `03` §9a стр.309 | **S-06 (ACCEPT)** |
...
- **A-01 — ✅ RESOLVED.** Каноническая формулировка найдена в старых файлах проекта: «Редакционные ошибки Roadmap и граф зависимостей» (версия 1.2 vs 1.1; пропуски пунктов 8/10 в Search Core не означают отсутствующие требования, карточки привязаны к названиям подэтапов; удалить техническое ребро EPIC-05 → EPIC-06, т.к. Change Log разрешает независимый backend; последовательность выдачи эпиков остаётся gated). Проверка: граф ацикличен, зависимости и обратные связи согласованы.
  **Диагноз:** не утрата данных, а **дефект миграции** — правило свёрнуто в `MASTER ROADMAP v1.2` Change Log п.12 с потерей источника, обоснования и критерия приёмки. Полный текст — `REVIEW-REGISTRY.md` §D-4.
...
| **Q-08** | ⚠ **ОТКРЫТ**: версионирование Document при смене URL | `07` §17, стр. 249 (помечен как открытый; ⟂ **собственного определения нет**) | `03` §9 стр.291; `04` §14a стр.312 | **S-12 (P0, ACCEPT) — блокер** |
...
| Structured output (2 уровня валидации) | `05` §10 | — | **N-03** |
...
| **D-3** | `A-01` входил в нормативный набор «УИ v1.0» без определения | приложения, 5 примечаний | ✅ **RESOLVED** — формулировка найдена в старых файлах, см. D-4 |
...
- id: A-01
  canonical: "старые файлы проекта — восстановлено (REVIEW-REGISTRY §D-4)"
  ...
  mentions: [ "EPIC SPECIFICATIONS стр.146", "ROADMAP стр.6, 244, 575", "EPIC→TASK стр.5, 269", "TASK EXEC стр.14" ]
  conflicts: []
  gates: []
  epics: [EPIC-06]
  status: active
  verdict: "D-4 (RESOLVED) — восстановить в нормативном составе, включая источник, обоснование и критерий приёмки"

- id: Q-08
  canonical: "07_QA_Acceptance.md §17 стр.249"
