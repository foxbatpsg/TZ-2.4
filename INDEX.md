...
|---|---|
| Основное ТЗ: `01`–`07` (7 файлов, ~211 КБ) | Карточки `приложения к ТЗ/Epic/` — `Глава_1…11_Карточки` (11 файлов, слой `E0X-C0Y`) |
...
> **Унификация определений (2026-09-15).** У каждой нормы `A-01…A-25`, `Q-01…Q-13` в корпусе — ровно **одно** определение; 53 применения переведены в ссылочную форму `` (`A-N`) `` (стиль уже применялся в `07` §28.1, стр. 575). Формулировки норм не редактировались. Аудит 2026-09-15 и счётчик 53 применения относятся к составу `A-01…A-21` (до внесения `A-22…A-25`); у норм `A-22…A-25` определение единственное с момента внесения. Класс «дубли определений» закрыт — см. §9 `D-4`.
...
- **О нумерации `D-N`.** Раздел §9 использует **локальную** нумерацию, которая с `корзина/REVIEW-REGISTRY.md` §2 не совпадает (совпадает только `D-6`) — см. §9.0.
...
| **Q-09** | Верификация авторов и статистическая выборка | `04` §16a, стр. 430 — норма «Политика верификации авторов и статистики (Q-09)» | карточки Epic: Глава 4, 5, 8, 10 | ✅ **CLOSED** (2026-09-15 — принято из черновика v1.1; база частично уже была в Q-03/профиле AGGREGATION); блокировки в карточках сняты 2026-09-18 |
| **Q-10** | Приватные данные в поисковых запросах (query egress) | `05` §3.3, стр. 83 — норма «Query egress policy (Q-10)» | `07` §24 стр.382; карточки Epic: Глава 4, 5, 6, 10 | ✅ **CLOSED** (2026-09-15 — принято из черновика v1.1, вариант A, default `PUBLIC_ONLY`) |
| **Q-11** | Перенос Study / откат snapshot | `06` §13, стр. 259 — норма «Перенос Study и откат (Q-11)» | карточки Epic: Глава 2, 8 | ✅ **CLOSED** (2026-09-15 — принято из черновика v1.1); блокировки в карточках сняты 2026-09-18 |
| **Q-12** | Целевая Windows-среда и performance acceptance | `02` §5.3, стр. 142 — норма «Целевая среда и performance budget (Q-12)» | `02` §5.3 стр.140; карточки Epic: Глава 8, 10, 11 | ✅ **CLOSED** (2026-09-15 — принято из черновика v1.1 как TARGET; численные критерии приёмки — только после benchmark-spike, S-13) |
...
| Общий Definition of Done TASK-карточек (`DOD-ALL`) | — | `TASK EXECUTION CONTRACT v1.0` §22 | — |
...
| **D-5** | `Q-13` использовался вне нормативного корпуса (карточки Epic) как блокер, но в реестре `Q-01…Q-08` отсутствовал | карточки Epic: 5 упоминаний | ✅ **CLOSED** (2026-09-13) — решение принято, diagnostic token снят из MVP. См. §4 `Q-13` |
