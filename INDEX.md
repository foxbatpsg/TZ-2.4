# INDEX.md — Навигационный слой ТЗ v2.6
...
| Приложения: `STATE MACHINE SPEC`, `BUDGET CONTRACT`, `MCP TOOL CONTRACTS`, `EPIC SPECIFICATIONS`, `EPIC → TASK DECOMPOSITION`, `MASTER ROADMAP`, `TASK EXECUTION CONTRACT` (7 файлов) | Файлы замечаний (отработаны 2026-09-15, архивированы в `корзина/`; вердикты — `REVIEW-REGISTRY.md` §1/§4/§5/§7), `README`/`CHANGELOG` |
...
> **Обновление статуса 2026-09-16:** по решению владельца `Q-06` и `Q-07` зарегистрированы как **OPEN** в `REVIEW-REGISTRY.md` §8 (ошибочная классификация «пропуск нумерации» снята). В тот же день решение `Q-06` **утверждено** (вариант A) и перенесено в нормативный корпус — строка ниже; перенос зафиксирован в `REVIEW-REGISTRY.md` §8 и `CHANGELOG.md` (v2.6). `Q-07` остаётся **OPEN**: проект — `OPEN-QUESTIONS-Q-06-Q-07.md` (не нормативный источник); зависимые задачи остаются `BLOCKED`.
