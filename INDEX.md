...
> **A-01 не имеет канонического определения** — см. §9.

| ID | Название | Первичный источник | Упоминания | Связь с реестром |
|---|---|---|---|---|
| **A-01** | ⟂ **ОПРЕДЕЛЕНИЕ ОТСУТСТВУЕТ** | — | EPIC SPEC стр.146, 146; ROADMAP стр.6, 244, 575; EPIC→TASK стр.5, 269; TASK EXEC стр.14 | **D-4 (OPEN)** |
...
- **A-01** — единственная норма без определения. Причина, по консистентности с `MASTER ROADMAP` стр.244 («пропуск номера ≠ отсутствие требования»), классифицируется как дефект ревизии, а не как умышленная группа норм. Предварительный вердикт: восстановить формулировку из пяти применений (все лежат в области интерпретации метаданных корпуса: версионирование документов, нумерация подэтапов, граф зависимостей EPIC).
...
| **Q-06…Q-07** | — | не найдены в корпусе | — | ⚠ пропуск нумерации |
| **Q-08** | ⚠ **ОТКРЫТ**: версионирование Document при смене URL | `07` §17, стр. 249 (помечен как открытый) | `03` §9 стр.291; `04` §14a стр.312 | **S-12 (P0, ACCEPT) — блокер** |
...
| **D-3** | `A-01` входит в нормативный набор «УИ v1.0», определения нет | приложения, 5 примечаний | **OPEN**, см. D-4 реестра |
...
- id: A-01
  ...
  epics: []
  status: defect
  verdict: "D-4 — восстановить формулировку из применений либо зафиксировать как дефект ревизии"
