...
- **Дубли с расхождением акцентов** (измерено 2026-09-15, `validate_index.py`): A-03 — 9 определений; A-01 — 7; A-11, A-16 — по 5; A-02, A-10, A-14, A-21, Q-01, Q-04 — по 4; A-04, A-09 — по 3; остальные — по 2.
- **Аудит дублей (2026-09-15): противоречий между вхождениями не обнаружено** (проверены все 21 норма). Все дополнительные вхождения — адресные пересказы нормы под локальным подзаголовком: A-11 — «Гарантии durability», «Мониторинг», «durability-режим»; A-20 — «Runtime stdout protection», «CI policy»; A-04 — «Область уникальности», «Меж-Study изоляция», «Область владения». Поле `canonical` («Первичный источник») заполнено для всех норм `A-01…A-25` и `Q-01…Q-13`; остальные вхождения переходят в `mentions`. См. §9 D-4.
...
| **Q-13** | Administrator diagnostic token | `EPIC SPECIFICATIONS v1.0` §12 — определение «Административный диагностический доступ (Q-13)» | `02` §7 стр.301; `MASTER ROADMAP v1.2` §14 стр.448; `EPIC→TASK` §13 стр.568 | ✅ **CLOSED** (2026-09-13) — **не вводится в MVP** |
...
| **D-4** | ⚠ Дубли определений (адресные пересказы нормы под локальным подзаголовком): A-03 ×9, A-01 ×7, A-11/A-16 ×5, A-02/A-10/A-14/A-21/Q-01/Q-04 ×4, A-04/A-09 ×3, прочие ×2 | `01`, `03`, `04`, `05`, `06`, `07`, приложения | ✅ **АУДИТ 2026-09-15** (`validate_index.py`): противоречий между вхождениями не обнаружено — проверены все 21 норма; `canonical` назначен для каждой нормы. Остаётся унификация локальных подзаголовков до канонического названия нормы — отдельным решением, не P0 |
...
| **D-6** | `Q-08`, `Q-10`, `Q-12`, `Q-13` — нормативный текст присутствует, но не приведён к обязательному шаблону `Название (Q-N): текст`; валидатор не распознавал определение (предупреждение [5] «нормы без канонического определения») | `03` §9b, `05` §3.3, `02` §5.3, `EPIC SPECIFICATIONS v1.0` §12 | ✅ **CLOSED** (2026-09-15) — определения приведены к шаблону без изменения смысла; валидатор: `РЕЗУЛЬТАТ: PASS — предупреждений 1`, блок [5] даёт `✓ у каждой нормы есть определение` |
