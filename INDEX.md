...
- **О нумерации `D-N`.** Раздел §9 использует **локальную** нумерацию, которая с `REVIEW-REGISTRY.md` §2 не совпадает (совпадает только `D-6`) — см. примечание в начале §9.
...
Нумерация `D-N` в этой таблице **локальна для `INDEX.md`** и совпадает с реестром `REVIEW-REGISTRY.md` только для `D-6`. Соответствия: `D-1` (здесь) — A-13 ↔ EPIC-10; `D-2` — `Q-06`/`Q-07`: открытые решения, не перенесённые в корпус (закрыт 2026-09-16 — оба перенесены); `D-3` — `A-01` без определения; `D-4` (здесь) — дубли определений норм. В `REVIEW-REGISTRY.md` `D-4` — это дефект «`A-01` не определён», а дубли определений зарегистрированы только здесь. Нормативные ID не переименовываются; коллизия снята примечаниями в обоих реестрах (`REVIEW-REGISTRY.md`, начало реестра).
