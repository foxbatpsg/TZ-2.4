...
дополнением к ТЗ, Утверждённым изменениям v1.0 (пакет УИ v1.0:
A-01…A-21, Q-01…Q-05; актуальный полный реестр норм — A-01…A-25,
Q-01…Q-13, см. INDEX.md §3/§4),
...
**`RELATED_ADR` (S-16):** идентификаторы архитектурных решений, от которых зависит задача (например, `ADR-001`). Если решений нет — явное значение `NONE`; пустое поле не допускается. Ссылка даётся по ID решения, не по номерам разделов.

**`BLOCKED_BY` (S-16):** идентификаторы блокеров задачи: ADR в статусе `Proposed`, другая TASK, отсутствующее решение. Любой `RELATED_ADR` в статусе `Proposed` автоматически блокирует зависимую реализацию — задача не запускается до перевода ADR в `Accepted`. Если блокеров нет — явное значение `NONE`.
...
**DOD-ALL — общий Definition of Done карточек `E0X-C0Y` (нормативная форма ссылки):**
`DOD-ALL` применяется к каждой TASK-карточке `приложения к ТЗ/Epic/` и означает
**полный неразделённый состав §22 п.1–10**: 1. GOAL реализован; 2. scope соблюдён;
3. запрещённые области не изменены; 4. tests созданы/обновлены; 5. tests проходят;
6. acceptance criteria проверены; 7. diff проверен; 8. нет незаявленных изменений;
9. архитектурные ограничения соблюдены; 10. результат передан в установленном формате.

Формулировки «Общий DoD: `DOD-ALL`» в шапках `Глава_1…11_Карточки` и токен `DOD-ALL`
в критериях приёмки карточек — **ссылка на настоящий пункт**. Сокращать («стабильная
часть», «из журнала» и подобные) либо переопределять состав `DOD-ALL` на уровне
карточки или главы запрещено; граница «стабильной части» не является нормой.
